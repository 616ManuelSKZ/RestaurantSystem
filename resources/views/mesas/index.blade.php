<x-app-layout>
    ...
    <div x-data="{ 
        ...
        @if($rol === 'administrador')
        {{-- Modal crear área --}}
        <div x-show="modalAreaCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
            x-cloak>
        ...
        {{-- Listado de Áreas --}}
        @foreach ($areas as $area)
        <div x-show="!filtroArea || filtroArea == '{{ $area->nombre }}'" x-transition
            ...
            {{-- Header del área --}}
            <div
                class="flex justify-between items-center px-6 py-4 bg-primary/5 dark:bg-primary/10 border-b border-primary/20 dark:border-primary/30">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div x-data="{ modalAreaEdit: false }">
                            <button @click="modalAreaEdit = true" class="text-blue-600 hover:text-blue-800 transition"
                                title="Editar área">
                            ...
                            <div x-show="modalAreaEdit"
                                ...
                                <div
                                    ...
                                    <h2 class="text-2xl font-semibold mb-4">Editar Área</h2>
                        ...
                        <div x-data="{ modalAreaDelete: false }" class="inline">
                            <button @click="modalAreaDelete = true" class="text-red-600 hover:text-red-800 transition"
                                title="Eliminar área">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-primary/10 dark:divide-primary/20">
                        ...
                        <tr class="hover:bg-primary/5 dark:hover:bg-primary/10 transition">
                            <td class="py-3 px-6 text-center text-gray-800 dark:text-gray-200 font-medium">
                                {{ $mesa->numero }}</td>
                            <td class="py-3 px-6 text-center text-gray-800 dark:text-gray-200">{{ $mesa->capacidad }}
                            </td>
                            <td class="py-3 px-6 text-center">
                                @if($mesa->estado === 'disponible')
                                <span
                                    class="px-3 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300">
                                    Disponible
                                </span>
                                @elseif($mesa->estado === 'ocupada')
                                <span
                                    class="px-3 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300">
                                    Ocupada
                                </span>
                                @else
                                <span
                                    class="px-3 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300">
                                    {{ $mesa->estado }}
                                </span>
                                @endif
                            </td>
                            ...
                            <td class="py-3 px-6 text-center space-x-3"
                                x-data="{ modalMesaDelete: false, mesaId: '{{ $mesa->id }}', mesaNombre: '{{ $mesa->numero ?? 'Sin número' }}' }">
                                ...
                                <button type="button" @click="modalMesaDelete = true"
