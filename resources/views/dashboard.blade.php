<x-app-layout>
    ...
    <main class="flex-1 p-8 bg-gray-50 dark:bg-background-dark/20 min-h-screen">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Total Ventas</h3>
                <select id="filtroVentas" class="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-1 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            <div class="bg-white dark:bg-background-dark/50 rounded-xl border border-primary/20 dark:border-primary/30 shadow-sm p-6">
                <p id="totalVentas" class="text-2xl font-bold text-gray-900 dark:text-white mb-4">
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-primary/10 dark:divide-primary/20">
                        @forelse($mesas as $mesa)
                            ...
                            @endphp
                            <tr class="hover:bg-primary/5 dark:hover:bg-primary/10">
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">Mesa {{ $mesa->numero ?? 'N/A' }}</td>
                                <td class="px-6 py-4 text-center">{{ $mesa->capacidad ?? 'N/A' }}</td>
                                <td class="px-6 py-4 text-center">
    ...
    <script>
        ...
        filtro.addEventListener('change', function() {
            ...
            totalVentasEl.textContent = `$${total.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
        });
    </script>
    @endpush
