<x-app-layout>
    ...
    <main class="flex-1 p-4 md:p-8 bg-gray-50 dark:bg-background-dark/20 min-h-screen">
        ...
        <div class="flex flex-col md:flex-row flex-wrap items-start md:items-end justify-between mb-4 gap-4">

            {{-- Filtros de búsqueda --}}
            <form method="GET" action="{{ route('facturas.index') }}" class="flex flex-col sm:flex-row flex-wrap gap-4 items-start md:items-end w-full md:w-auto">
                {{-- Buscar por ID --}}
                ...
                <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    ...
                    <a href="{{ route('facturas.index') }}"
                        class="px-4 py-2 bg-gray-400 text-white rounded-lg shadow hover:bg-gray-500 transition w-full sm:w-auto text-center">
                        Limpiar
                    </a>
                </div>
            </form>
            ...
            <div x-data="{ modalGenerarResumen: false }" class="w-full md:w-auto">
                <button @click="modalGenerarResumen = true"
                    class="px-4 py-2 bg-green-600 text-white rounded-lg shadow hover:bg-green-700 transition w-full">
                    Generar Resumen PDF
                </button>
                ...
                <div x-show="modalGenerarResumen"
                    ...
                    x-transition>
                    <div class="bg-white dark:bg-background-dark/100 rounded-lg shadow-lg w-full max-w-md p-6 mx-4"
        ...
        {{-- Tabla de Facturas --}}
        <div
            class="bg-white dark:bg-background-dark/50 rounded-xl border border-primary/20 dark:border-primary/30 overflow-hidden shadow-sm">
            @if ($facturas->isEmpty())
            <p class="px-4 md:px-6 py-6 text-center text-gray-500 dark:text-gray-400">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left min-w-[600px]">
                    <thead class="bg-primary/5 dark:bg-primary/10 text-gray-600 dark:text-gray-300 uppercase text-xs">
                        <tr>
                            <th class="px-4 md:px-6 py-3 font-medium text-center">ID</th>
                            <th class="px-4 md:px-6 py-3 font-medium text-center">Cliente</th>
                            <th class="px-4 md:px-6 py-3 font-medium text-center">Tipo</th>
                            <th class="px-4 md:px-6 py-3 font-medium text-center">Fecha</th>
                            <th class="px-4 md:px-6 py-3 font-medium text-right">Total</th>
                    ...
                    <tbody class="divide-y divide-primary/10 dark:divide-primary/20">
                        ...
                        <tr class="hover:bg-primary/5 dark:hover:bg-primary/10 transition">
                            <td class="px-4 md:px-6 py-4 font-semibold text-gray-900 dark:text-white text-center">
                                #{{ $factura->id }}
                            </td>
                            <td class="px-4 md:px-6 py-4 text-gray-800 dark:text-gray-300 text-center">
                                {{ $factura->nombre_cliente ?? 'Sin cliente' }}
                            </td>
                            <td class="px-4 md:px-6 py-4 text-gray-800 dark:text-gray-300 text-center">
                                {{ $factura->tipo_factura }}
                            </td>
                            <td class="px-4 md:px-6 py-4 text-center text-gray-700 dark:text-gray-300">
                                <div class="font-semibold text-indigo-600 dark:text-indigo-400">
                                    {{ \Carbon\Carbon::parse($factura->fecha_emision)->format('H:i') }}
                                </div>
                                <div class="text-gray-800 dark:text-gray-300 text-sm">
                                    {{ \Carbon\Carbon::parse($factura->fecha_emision)->format('d/m/Y') }}
                                </div>
                            </td>
                            <td class="px-4 md:px-6 py-4 text-right font-semibold text-gray-900 dark:text-white">
                                ${{ number_format($factura->total, 2) }}
                            </td>
                            <td class="px-4 md:px-6 py-4 text-center space-x-1 md:space-x-2">
                                <a href="{{ route('facturas.show', $factura) }}"
                                    class="inline-block px-2 md:px-3 py-1 text-xs font-semibold rounded-lg bg-blue-600 hover:bg-blue-700 text-white shadow-sm transition">
                                    Ver
                                </a>
                                <a href="{{ route('facturas.exportPDF', $factura) }}" target="_blank"
                                    class="inline-block px-2 md:px-3 py-1 text-xs font-semibold rounded-lg bg-red-600 hover:bg-red-700 text-white shadow-sm transition">
                                    PDF
                                </a>
