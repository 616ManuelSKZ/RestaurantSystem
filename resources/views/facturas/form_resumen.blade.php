<form action="{{ route('facturas.pdf.resumen') }}" method="GET" target="_blank" class="mb-6 flex flex-wrap gap-4 items-end">
    <div class="w-1/2">
    ...
    <div class="mt-4 flex flex-wrap gap-2">
        <a href="{{ route('facturas.pdf.resumen', ['periodo' => 'dia']) }}" target="_blank"
            class="px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition">
            PDF Día
        </a>
        <a href="{{ route('facturas.pdf.resumen', ['periodo' => 'semana']) }}" target="_blank"
            class="px-4 py-2 bg-green-600 text-white rounded-lg shadow hover:bg-green-700 transition">
            PDF Semana
        </a>
        <a href="{{ route('facturas.pdf.resumen', ['periodo' => 'mes']) }}" target="_blank"
            class="px-4 py-2 bg-yellow-600 text-white rounded-lg shadow hover:bg-yellow-700 transition">
            PDF Mes
        </a>
        <a href="{{ route('facturas.pdf.resumen', ['periodo' => 'anio']) }}" target="_blank"
            class="px-4 py-2 bg-red-600 text-white rounded-lg shadow hover:bg-red-700 transition">
            PDF Año
        </a>
        <a href="{{ route('facturas.pdf.resumen', ['periodo' => 'general']) }}" target="_blank"
