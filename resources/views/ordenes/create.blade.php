<x-app-layout>
    ...
    <div class="min-h-screen flex" x-data="{ sidebarOpen: false }">
        ...
        <main class="flex-1 flex flex-col p-4 md:p-6 overflow-y-auto md:ml-0">
            ...
            <h2 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-4">Nueva Orden</h2>
            <p class="text-base md:text-lg text-gray-500 dark:text-gray-400 mb-6">Selecciona área, mesa y platillos</p>

            <form action="{{ route('ordenes.store') }}" method="POST" class="bg-background-light dark:bg-background-dark rounded-xl border border-primary/20 dark:border-primary/30 shadow-sm flex-1 flex flex-col p-4 md:p-6">
                @csrf
                ...
                <div class="flex flex-col md:flex-row gap-4 mb-4">
                    <div class="flex-1">
                        ...
                        <select name="id_area_mesas" id="id_area_mesas" class="w-full border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm bg-white dark:bg-background-dark/50 text-gray-900 dark:text-white" required>
                    ...
                    <div class="flex-1">
                        ...
                        <select name="id_mesas" id="id_mesas" class="w-full border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm bg-white dark:bg-background-dark/50 text-gray-900 dark:text-white" required>
    ...
    <script>
        let count = 0;
        const platillosSeleccionados = {}; // Guarda los platillos ya seleccionados
        ...
        function addPlatillo(id, nombre, precio) {
            ...
            // Si ya existe el platillo, solo incrementa su cantidad
            ...
            const html = `
                <div id="platillo-${id}" class="flex gap-2 items-center" data-precio="${precio}">
                    ...
                    <div class="flex items-center border rounded-md overflow-hidden">
                        ...
                        <input type="text" readonly name="platillos[${count}][cantidad]" value="1" 
                            class="cantidad w-12 text-center bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                        <button type="button" onclick="cambiarCantidad(${id}, 1)" class="px-2 py-1 bg-gray-200 dark:bg-gray-600">+</button>
                    </div>
        ...
        function eliminarPlatillo(id) {
            ...
            delete platillosSeleccionados[id]; // 🔹 Permitir volver a agregarlo
            actualizarResumen();
        }

        // Cargar mesas según área seleccionada (igual que antes)
                    ...
                    .then(data => {
                        ...
                        data.forEach(menu => {
                            ...
                            div.addEventListener('click', () => {
                                addPlatillo(menu.id, menu.nombre, parseFloat(menu.precio));
                            });
