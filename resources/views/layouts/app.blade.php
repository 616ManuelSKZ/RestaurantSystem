...
<body class="bg-background-light dark:bg-background-dark font-display text-text-light dark:text-text-dark relative">
    ...
    <!-- Contenedor principal con sidebar fijo -->
    <div class="min-h-screen flex" x-data="{ sidebarOpen: false }">
        {{-- Sidebar Izquierdo (fijo en toda la pantalla) --}}
        <aside
            :class="{ 'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen }"
            class="fixed left-0 top-0 h-screen z-40 w-64 bg-background-light dark:bg-background-dark shadow-md flex flex-col text-text-light dark:text-text-dark transform transition-transform duration-300 ease-in-out md:translate-x-0">
            ...
            <div class="p-4 border-b border-subtle-light dark:border-subtle-dark flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-placeholder-light dark:text-placeholder-dark capitalize">
                        {{ Auth::user()->rol }}</p>
                </div>
            </div>
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto" x-data="{ openOrdenes: false, openAjustes: false }">
                <!-- Dashboard -->
                ...
                <!-- Órdenes -->
                @if(in_array($rol, ['administrador','mesero','cajero', 'cocinero']))
                <button @click="openOrdenes = !openOrdenes"
                    ...
                    📝 Órdenes
                    <svg :class="{ 'rotate-180': openOrdenes }" class="w-4 h-4 transition-transform" fill="none"
                ...
                <div x-show="openOrdenes" x-collapse class="ml-6 space-y-1">
                    ...
                    <a href="{{ route('ordenes.create') }}"
                        class="block px-4 py-2 rounded-lg hover:bg-subtle-light dark:hover:bg-subtle-dark">
                        ➕ Nueva Orden
                    </a>
                    @endif
                    @if(in_array($rol, ['administrador','cajero','mesero','cocinero']))
                ...
                <button @click="openAjustes = !openAjustes"
                    class="w-full flex items-center justify-between px-4 py-2 rounded-lg hover:bg-subtle-light dark:hover:bg-subtle-dark transition">
                    ⚙️ Ajustes
                    <svg :class="{ 'rotate-180': openAjustes }" class="w-4 h-4 transition-transform" fill="none"
                        stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                    </svg>
                </button>
            ...
            <div class="sticky bottom-0 p-4 border-t border-subtle-light dark:border-subtle-dark bg-background-light dark:bg-background-dark">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="w-full text-left px-4 py-2 rounded-lg hover:bg-subtle-light dark:hover:bg-subtle-dark text-red-600">
                        🚪 Cerrar sesión
                    </button>
                </form>
            </div>
        ...
        <div class="flex-1 flex flex-col md:ml-64">
            ...
            <header class="bg-background-light dark:bg-background-dark shadow">
                <div class="px-6 py-4 flex items-center justify-between">
                    ...
                    <button @click="sidebarOpen = !sidebarOpen" class="md:hidden p-2 rounded-lg hover:bg-subtle-light dark:hover:bg-subtle-dark">
            ...
            {{-- Footer --}}
            <footer
                class="bg-background-light dark:bg-background-dark border-t border-subtle-light dark:border-subtle-dark">
                <div class="px-6 py-4 flex justify-between text-sm text-placeholder-light dark:text-placeholder-dark">
                    <p>&copy; {{ date('Y') }} {{ config('app.name', 'Restaurante') }}. Todos los derechos reservados.
                    </p>
